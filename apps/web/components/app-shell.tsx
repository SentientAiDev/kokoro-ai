...
const navItems = [{ href: '/today', label: 'Today' }];
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col px-6 py-6">
        <header className="mb-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isGuest ? <Badge>Guest (not synced)</Badge> : null}
            ...
            <details className="relative">
              ...
              <div className="absolute right-0 z-10 mt-2 w-56 rounded-xl border border-border bg-white p-2 shadow-md">
                ...
                <div className="mx-2 mt-2 border-t pt-2">
                  <p className="mb-1 px-1 text-xs text-muted-foreground">{userLabel}</p>
                  {isGuest ? (
                    <Link href="/login" className="block rounded-md px-3 py-2 text-sm text-slate-700 hover:bg-slate-100">
                      Sign in to sync
                    </Link>
                  ) : (
